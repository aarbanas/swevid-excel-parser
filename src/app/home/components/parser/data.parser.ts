...
} from '../invalid-cell-modal/invalid.cell.modal.component'
...
export class DataParser {
  ...
  async parse (json: any, sheet: string, starting_row: number, name_cell: string, sex_cell: string, year_of_birth_cell: string, disciplines: {
    [cell: string]: any
  }, organisation: any, swevid_path: string): Promise<{
    success: boolean,
    error?: string
  }> {
    let counter = 0
    ...
    for (const [index, row] of json[sheet].entries()) {
      ...
      if (validationObject.name === false || validationObject.birthDate === false) {
        const { name: _name, birthDate: _birthDate } = await this.openValidationModal(name, birthDate, validationObject)
        name = _name
  ...
  openValidationModal (name: string, birthDate: string, validationObject: any): Promise<{ name: string, birthDate: string }> {
  ...
  private formatDateBirth (dateBirth: string): string {
    ...
    if (dateBirth.length === 2) {
      ...
      if (_convertedYear <= year)
        return _convertedYear.toString()
